...
class Router
{
    ...
    public function findRoute(string $name): ?ILoadableRoute
    {
        ...
        foreach ($this->processedRoutes as $route) {
            ...
            /* Check if the name matches with a name on the route. Should match either router alias or controllers alias. */
            ...
            /* Direct match to controllers */
            if ($route instanceof IControllerRoute && strtoupper($route->getController()) === strtoupper($name)) {
                $this->debug('Found route "%s" by controllers "%s"', $route->getUrl(), $name);

                return $route;
            }

            /* Using @ is most definitely a controllers@method or alias@method */
            if (strpos($name, '@') !== false) {
                [$controller, $method] = array_map('strtolower', explode('@', $name));

                if ($controller === strtolower($route->getClass()) && $method === strtolower($route->getMethod())) {
                    $this->debug('Found route "%s" by controllers "%s" and method "%s"', $route->getUrl(), $controller, $method);
    ...
    public function getUrl(?string $name = null, $parameters = null, ?array $getParams = null): Url
    {
        ...
        if (is_string($name) === true && strpos($name, '@') !== false) {
            ...
            foreach ($this->processedRoutes as $processedRoute) {
                ...
                /* Check if the route controllers is equal to the name */
